...
#ifndef SPIF_USE_4BYTE_ADDRESSES
/** Use 4 byte addresses (comment out/set to 0 to use 3 byte addresses) */
#define SPIF_USE_4BYTE_ADDRESSES 1
#endif
...
#if SPIF_USE_4BYTE_ADDRESSES
/**
 * @brief Table of registers for 4-byte address instructions
 *
 * Adopted from section 9.1.1 in S25FS512S datasheet
 * https://www.infineon.com/dgdl/Infineon-S25FS512S_512_Mb_1.8_V_Serial_Peripheral_Interface_with_Multi-I_O_Non-Volatile_Flash-DataSheet-v14_00-EN.pdf?fileId=8ac78c8c7d0d8da4017d0ed681a356fe
 */
enum spif_four_byte_instructions {
    SPIF_4READ = 0x13, // Read
    SPIF_4FAST_READ = 0x0C, // Read Fast
    SPIF_4DIOR = 0xBC, // Dual I/O Read
    SPIF_4QIOR = 0xEC, // Quad I/O Read
    SPIF_4DDRQIOR = 0xEE, // DDR Quad I/O Read
    SPIF_4PP = 0x12, // Page Program
    SPIF_4P4E = 0x21, // Parameter 4-KB Erase
    SPIF_4SE = 0xDC, // Erase 64 / 256 KB
    SPIF_4ECCRD = 0x18, // ECC Status Read
    SPIF_4DYBRD = 0xE0, // DYB Read
    SPIF_4DYBWR = 0xE1, // DYBWR
    SPIF_4PPBRD = 0xE2, // PPB Read
    SPIF_4PPBP = 0xE3, // PPB Program
};
#endif
...
#if SPIF_USE_4BYTE_ADDRESSES
    _address_size = SPIF_ADDR_SIZE_4_BYTES;         // Enable 4-byte addressing
#else
...
#else
        /* Select the proper erase instruction and size based on the size of the current region */
        if (_sfdp_info.smptbl.region_size[region] > 0x1000)
        {
            /* Use 256 KB sector erase */
            type = 2;
            cur_erase_inst = SPIF_4SE;
            curr_erase_size = 0x40000; /* 4SE always erases 256 KB */
        }
        else
        {
            /* Use 4 KB parameter sector erase */
            type = 1;
            cur_erase_inst = SPIF_4P4E;
            curr_erase_size = 0x1000; /* 4P4E erases a 4 KB parameter sector */
        }
#endif
...
int SPIFBlockDevice::bulk_erase()
{
    if (!_is_initialized) {
        return BD_ERROR_DEVICE_ERROR;
    }
    ...
    /*
     * See section 9.6.3 of the S25FS512S datasheet
     * https://www.infineon.com/dgdl/Infineon-S25FS512S_512_Mb_1.8_V_Serial_Peripheral_Interface_with_Multi-I_O_Non-Volatile_Flash-DataSheet-v14_00-EN.pdf?fileId=8ac78c8c7d0d8da4017d0ed681a356fe
     *
     * Before the BE instruction can be accepted, WREN must be issued and decoded, setting WEL.
     *
     * CS must be driven HIGH after sending the BE instruction, which initiates the erase cycle.
     *
     * While the erase is in progress, the WIP bit is set to 1, and the bit is set to 0 once the
     * erase has completed.
     *
     * BE can only be performed when Block Protection is disabled.
     *
     * t_BE is typically 220 seconds, with a maximum time of 720 seconds.
     */
    static constexpr int SPIF_BE = 0x60;

    static constexpr auto TYP_BULK_ERASE_TIME = 220s;
    static constexpr auto MAX_BULK_ERASE_TIME = 720s;
    static constexpr auto SINGLE_MEM_READY_MAX_TIME = 1ms * IS_MEM_READY_MAX_RETRIES;
    ...
    /* Send the BE instruction */
    if (SPIF_BD_ERROR_OK
        != _spi_send_general_command(SPIF_BE, SPI_NO_ADDRESS_COMMAND, NULL, 0, NULL, 0))
    {
        tr_error("Sending BE command FAILED");
        status = SPIF_BD_ERROR_READY_FAILED;
        goto exit_point;
    }
    ...
    {
        tr_info("Bulk Erase took more than %dms", IS_MEM_READY_MAX_RETRIES * ++wip_retries);

        if (wip_retries > (TYP_BULK_ERASE_TIME / SINGLE_MEM_READY_MAX_TIME))
        {
            tr_warn("Bulk Erase has taken longer than typical time: %dms",
                wip_retries * IS_MEM_READY_MAX_RETRIES);
        }

        if (wip_retries > (MAX_BULK_ERASE_TIME / SINGLE_MEM_READY_MAX_TIME))
        {
            tr_error("Bulk Erase took too long, aborting!");
            status = SPIF_BD_ERROR_READY_FAILED;
            goto exit_point;
        }
    }
...
#if SPIF_USE_4BYTE_ADDRESSES
    _read_instruction = SPIF_4READ;
    _prog_instruction = SPIF_4PP;
    _erase_instruction = SPIF_4SE;
#else
